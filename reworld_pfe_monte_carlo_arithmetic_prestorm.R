...
cat("========================================\n")
cat("ReWorld Holdings PFE Model (ARITHMETIC RETURNS)\n")
cat("========================================\n\n")
...
# -----------------------------------------------------------
# 4. CURRENT FORWARD CURVE + ARITHMETIC RETURN VOLATILITY
# -----------------------------------------------------------
...
# Build ARITHMETIC RETURNS: (P_t - P_{t-1}) / P_{t-1}
historical_changes <- historical_prices %>%
  arrange(type, maturity_date, as_of_date) %>%
  group_by(type, maturity_date) %>%
  mutate(arith_return = (price - lag(price)) / lag(price)) %>%
  ungroup()

# Trailing volatility (30-day) using arithmetic returns
vol_summary <- historical_changes %>%
  filter(!is.na(arith_return)) %>%
  group_by(type, maturity_date) %>%
  arrange(as_of_date, .by_group = TRUE) %>%
  slice_tail(n = cfg$vol_lookback_n) %>%
  summarise(
    sd_arith_return = sd(arith_return, na.rm = TRUE),
    n_obs_used      = n(),
    .groups         = "drop"
  ) %>%
  mutate(
    ann_factor            = sqrt(252),
    sd_arith_return_ann   = sd_arith_return * ann_factor,
    sd_arith_return_monthly = sd_arith_return_ann / sqrt(12)
  )
...
cat("\n========================================\n")
cat("VOLATILITY SUMMARY (ARITHMETIC RETURNS)\n")
cat("========================================\n")

cat("\nPeak volatilities (first 12):\n")
forward_vol_table %>%
  filter(type == "Peak") %>%
  dplyr::select(maturity_date, price, sd_arith_return_ann, sd_arith_return_monthly, n_obs_used) %>%
  head(12) %>%
  print()
...
cat("Peak:", round(mean(vol_summary$sd_arith_return_ann[vol_summary$type == "Peak"], na.rm = TRUE) * 100, 2), "%\n")
cat("Off-Peak:", round(mean(vol_summary$sd_arith_return_ann[vol_summary$type == "Off-Peak"], na.rm = TRUE) * 100, 2), "%\n")

# -----------------------------------------------------------
# 5. BUILD CORRELATION MATRIX FROM ARITHMETIC RETURNS
# -----------------------------------------------------------
...
returns_wide <- historical_changes %>%
  filter(!is.na(arith_return)) %>%
  dplyr::select(as_of_date, contract_id, arith_return) %>%
  pivot_wider(names_from = contract_id, values_from = arith_return) %>%
  arrange(as_of_date)
...
peak_contracts <- volumes %>%
  inner_join(
    forward_vol_table %>% filter(type == "Peak") %>% dplyr::select(maturity_date, price, sd_arith_return_monthly),
    by = "maturity_date"
  ) %>%
  transmute(
    ...
    mwhrs = peak_mwh,
    strike = price,
    monthly_vol = sd_arith_return_monthly,  # % vol
...
offpeak_contracts <- volumes %>%
  inner_join(
    forward_vol_table %>% filter(type == "Off-Peak") %>% dplyr::select(maturity_date, price, sd_arith_return_monthly),
    by = "maturity_date"
  ) %>%
  transmute(
...
# -----------------------------------------------------------
# 8. PATH-BASED MONTE CARLO SIMULATION (ARITHMETIC RETURNS)
# -----------------------------------------------------------
...
cat("\n========================================\n")
cat("SIMULATION SETUP (ARITHMETIC RETURNS)\n")
cat("========================================\n")
...
# Get contract IDs and volatilities
contract_ids <- contracts$contract_id
monthly_vols <- contracts$monthly_vol  # % vol

cat("\nSimulating", n_sims, "paths across", n_periods, "months (arithmetic returns)...\n")
...
for (t in 1:n_periods) {
  ...
  # Update prices for active contracts - ARITHMETIC (multiplicative with linear returns)
  for (i in 1:n_active) {
    idx <- active_idx[i]
    sigma_m <- monthly_vols[idx]  # % vol
    ...
    # ARITHMETIC RETURN evolution: P_new = P_old * (1 + drift + sigma * Z)
    sim_prices[, idx] <- sim_prices[, idx] * (1 + drift_monthly + sigma_m * Z[, i])
...
p_pfe <- ggplot(pfe_profile, aes(x = margin_date)) +
  ...
  labs(
    title = "ReWorld Holdings - PFE Margin Profile (Arithmetic Returns)",
...
p_vm <- ggplot(pfe_profile, aes(x = margin_date)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(scale = 1/1e6, suffix = "M")) +
  labs(
    title = "ReWorld Holdings - PFE Profile (Arithmetic Returns)",
...
p_peak <- ggplot(peak_pfe, aes(x = margin_date)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(scale = 1/1e6, suffix = "M")) +
  labs(
    title = "ReWorld Holdings - Peak PFE Profile (Arithmetic Returns)",
...
p_offpeak <- ggplot(offpeak_pfe, aes(x = margin_date)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(scale = 1/1e6, suffix = "M")) +
  labs(
    title = "ReWorld Holdings - Off-Peak PFE Profile (Arithmetic Returns)",
...
p_combined <- ggplot(combined_p95, aes(x = margin_date, y = P95, color = Type, linetype = Type)) +
  ...
  labs(
    title = "ReWorld Holdings - Peak vs Off-Peak P95 Comparison (Arithmetic Returns)",
    subtitle = "MTM gain to SEUS",
    x = "Margin Date",
...
p_stacked <- ggplot(stacked_data, aes(x = margin_date, y = P95, fill = Type)) +
  ...
  labs(
    title = "ReWorld Holdings - Stacked P95 Exposure by Type (Arithmetic Returns)",
...
p_curve <- ggplot(peak_detail, aes(x = maturity_date)) +
  ...
  scale_fill_manual(values = c("Peak" = "red", "Off-Peak" = "steelblue")) +
  labs(
    title = paste("Forward Curves at Peak Margin Date:", max_vm_date, "(Arithmetic Returns)"),
...
# Export PFE profile
write_csv(pfe_profile, paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, "_arithmetic.csv"))
cat("\nSaved:", paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, "_arithmetic.csv"), "\n")

# Export peak detail
write_csv(peak_detail, paste0(cfg$file_path, "reworld_peak_margin_detail", cfg$output_suffix, "_arithmetic.csv"))
cat("Saved:", paste0(cfg$file_path, "reworld_peak_margin_detail", cfg$output_suffix, "_arithmetic.csv"), "\n")

# Export contracts
write_csv(contracts, paste0(cfg$file_path, "reworld_contracts", cfg$output_suffix, "_arithmetic.csv"))
cat("Saved:", paste0(cfg$file_path, "reworld_contracts", cfg$output_suffix, "_arithmetic.csv"), "\n")
...
contract_summary <- contract_summary %>%
  mutate(
    ...
    price_p50 = price_p50,
    price_p95 = price_p95,
    ann_vol = monthly_vol * sqrt(12),  # Annualized %
...
write_csv(contract_summary, paste0(cfg$file_path, "reworld_contract_summary", cfg$output_suffix, "_arithmetic.csv"))
cat("Saved:", paste0(cfg$file_path, "reworld_contract_summary", cfg$output_suffix, "_arithmetic.csv"), "\n")
...
# Save plots
ggsave(paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, "_arithmetic.png"), p_pfe, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_vm_only", cfg$output_suffix, "_arithmetic.png"), p_vm, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_vm_only", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_peak_only", cfg$output_suffix, "_arithmetic.png"), p_peak, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_peak_only", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_offpeak_only", cfg$output_suffix, "_arithmetic.png"), p_offpeak, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_offpeak_only", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_peak_vs_offpeak", cfg$output_suffix, "_arithmetic.png"), p_combined, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_peak_vs_offpeak", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_stacked", cfg$output_suffix, "_arithmetic.png"), p_stacked, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_stacked", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_mwh_profile", cfg$output_suffix, "_arithmetic.png"), p_mwh, width = 12, height = 6, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_mwh_profile", cfg$output_suffix, "_arithmetic.png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_forward_curve_at_peak", cfg$output_suffix, "_arithmetic.png"), p_curve, width = 12, height = 6, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_forward_curve_at_peak", cfg$output_suffix, "_arithmetic.png"), "\n")

cat("\n========================================\n")
cat("MODEL COMPLETE (ARITHMETIC RETURNS)\n")
cat("========================================\n")
