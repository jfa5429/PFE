...
cat("========================================\n")
cat("ReWorld Holdings PFE Model\n")
cat("========================================\n\n")
...
forward_vol_table %>%
  filter(type == "Peak") %>%
  dplyr::select(maturity_date, price, sd_log_return_ann, sd_log_return_monthly, n_obs_used) %>%
  head(12) %>%
  print()
...
cat("Peak:", round(mean(vol_summary$sd_log_return_ann[vol_summary$type == "Peak"], na.rm = TRUE) * 100, 2), "%\n")
cat("Off-Peak:", round(mean(vol_summary$sd_log_return_ann[vol_summary$type == "Off-Peak"], na.rm = TRUE) * 100, 2), "%\n")
...
peak_contracts <- volumes %>%
  inner_join(
    forward_vol_table %>% filter(type == "Peak") %>% dplyr::select(maturity_date, price, sd_log_return_monthly),
    by = "maturity_date"
  ) %>%
  transmute(
    ...
    mwhrs = peak_mwh,
    strike = price,
    monthly_vol = sd_log_return_monthly,
...
offpeak_contracts <- volumes %>%
  inner_join(
    forward_vol_table %>% filter(type == "Off-Peak") %>% dplyr::select(maturity_date, price, sd_log_return_monthly),
    by = "maturity_date"
  ) %>%
  transmute(
...
# -----------------------------------------------------------
# 8. PATH-BASED MONTE CARLO SIMULATION
# -----------------------------------------------------------
...
cat("\nSimulating", n_sims, "paths across", n_periods, "months...\n")
...
p_pfe <- ggplot(pfe_profile, aes(x = margin_date)) +
  ...
  labs(
    title = "ReWorld Holdings - PFE Margin Profile",
...
p_vm <- ggplot(pfe_profile, aes(x = margin_date)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(scale = 1/1e6, suffix = "M")) +
  labs(
    title = "ReWorld Holdings - PFE Profile",
...
p_peak <- ggplot(peak_pfe, aes(x = margin_date)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(scale = 1/1e6, suffix = "M")) +
  labs(
    title = "ReWorld Holdings - Peak PFE Profile",
...
p_offpeak <- ggplot(offpeak_pfe, aes(x = margin_date)) +
  ...
  scale_y_continuous(labels = scales::dollar_format(scale = 1/1e6, suffix = "M")) +
  labs(
    title = "ReWorld Holdings - Off-Peak PFE Profile",
...
p_combined <- ggplot(combined_p95, aes(x = margin_date, y = P95, color = Type, linetype = Type)) +
  ...
  labs(
    title = "ReWorld Holdings - Peak vs Off-Peak P95 Comparison",
    subtitle = "MTM gain to SEUS",
    x = "Margin Date",
...
p_stacked <- ggplot(stacked_data, aes(x = margin_date, y = P95, fill = Type)) +
  ...
  labs(
    title = "ReWorld Holdings - Stacked P95 Exposure by Type",
...
p_curve <- ggplot(peak_detail, aes(x = maturity_date)) +
  ...
  scale_fill_manual(values = c("Peak" = "red", "Off-Peak" = "steelblue")) +
  labs(
    title = paste("Forward Curves at Peak Margin Date:", max_vm_date),
...
# Export PFE profile
write_csv(pfe_profile, paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, ".csv"))
cat("\nSaved:", paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, ".csv"), "\n")

# Export peak detail
write_csv(peak_detail, paste0(cfg$file_path, "reworld_peak_margin_detail", cfg$output_suffix, ".csv"))
cat("Saved:", paste0(cfg$file_path, "reworld_peak_margin_detail", cfg$output_suffix, ".csv"), "\n")

# Export contracts
write_csv(contracts, paste0(cfg$file_path, "reworld_contracts", cfg$output_suffix, ".csv"))
cat("Saved:", paste0(cfg$file_path, "reworld_contracts", cfg$output_suffix, ".csv"), "\n")
...
write_csv(contract_summary, paste0(cfg$file_path, "reworld_contract_summary", cfg$output_suffix, ".csv"))
cat("Saved:", paste0(cfg$file_path, "reworld_contract_summary", cfg$output_suffix, ".csv"), "\n")
...
# Save plots
ggsave(paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, ".png"), p_pfe, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_profile", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_vm_only", cfg$output_suffix, ".png"), p_vm, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_vm_only", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_peak_only", cfg$output_suffix, ".png"), p_peak, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_peak_only", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_offpeak_only", cfg$output_suffix, ".png"), p_offpeak, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_offpeak_only", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_peak_vs_offpeak", cfg$output_suffix, ".png"), p_combined, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_peak_vs_offpeak", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_pfe_stacked", cfg$output_suffix, ".png"), p_stacked, width = 12, height = 7, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_pfe_stacked", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_mwh_profile", cfg$output_suffix, ".png"), p_mwh, width = 12, height = 6, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_mwh_profile", cfg$output_suffix, ".png"), "\n")

ggsave(paste0(cfg$file_path, "reworld_forward_curve_at_peak", cfg$output_suffix, ".png"), p_curve, width = 12, height = 6, dpi = 150)
cat("Saved:", paste0(cfg$file_path, "reworld_forward_curve_at_peak", cfg$output_suffix, ".png"), "\n")
